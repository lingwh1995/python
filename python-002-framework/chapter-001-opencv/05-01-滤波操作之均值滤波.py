...
def opencv_blur_operation():
    """
        ...
        cv2.blur() 函数用于对图像进行均值模糊。

        blur(src, ksize[, dst[, anchor[, borderType]]])
        - src ：输入的图像。
        - ksize ：均值滤波核的大小。必须是正的奇数。
        - dst ：输出图像，与输入图像具有相同的尺寸和类型。
        - anchor ：锚点，指定了核的中心位置。默认值为 (-1, -1)，表示核的中心在核的中心位置。
        - borderType ：边界模式，定义了图像边界外像素的处理方式。 默认值为 cv2.BORDER_DEFAULT。
    """
    # 读取RGB格式图像
    ...
    kernel_size = 2
    # 均值滤波
    image_mean_blur = cv2.blur(image, (kernel_size, kernel_size))

    # 或使用 boxFilter
    # image_mean_blur = cv2.boxFilter(image, -1, (kernel_size, kernel_size))

    # 显示图像
    image_util.show_image_in_window('均值滤波后的图像', image_mean_blur)

    # 保存图像
    # cv2.imwrite(output_path, image_mean_blur)
